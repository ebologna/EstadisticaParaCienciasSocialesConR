...
```{r radios, fig.cap="Radios censales 2010 de la ciudad de Río Cuarto, provincia de Córdoba, Argentina.", fig.align='center', out.width='80%'}
...
[http://www.respondentdrivensampling.org/](http://www.respondentdrivensampling.org/), donde se encuentran abundantes ejemplos de aplicación.
...
```{r echo=TRUE}
eph.3.18 <- read.csv("bases/archivostxt/usu_individual_T318.txt",
                     sep = ";")
...
```{r echo=TRUE}
addmargins(table(muestra.estrat.eph$CH04,
                 muestra.estrat.eph$NIVEL_ED))
```
...
```{r echo=TRUE}
muestra.estrat.prop.eph <- stratified(eph.3.18,
                                      "NIVEL_ED", .1)

addmargins(
  table(muestra.estrat.prop.eph$CH04,
        muestra.estrat.prop.eph$NIVEL_ED)
)
```
