...
Si se trata de una variable de mayor nivel de medición, no hay ninguna diferencia. La variable *concepto asignado a estudiantes por parte de sus docentes*, que es de nivel ordinal, tiene la distribución de frecuencias siguiente:  
...
distribución. El *número de materias que tienen aprobadas quienes han terminado de cursar el primer año* de su carrera se distribuye así:
...
```{r fig.cap= "Distribución del  número de aciertos", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
...
La **mediana** es el cuantil $0.50$ o bien  el percentil $50$. Se define como  el valor que *supera a* y *es superado por* no más de la mitad de las observaciones. Es el valor que deja por debajo de sí y por encima de sí *a lo sumo* la mitad de las observaciones. Es importante tener en cuenta que se trata de la mitad de los casos y no la mitad de las categorías.  La siguiente distribución presenta en serie simple ordenada el número de sesiones de psicoterapia que recibieron 9 pacientes en un hospital:
...
El valor 7 es la mediana, porque es *superado* por tres
valores (10, 15 y 19), que es menos de la mitad; y *supera* a dos (2 y 3), que es menos de la mitad.
...
una variable cuantitativa discreta. Si la variable es el *número de síntomas* a partir de los cuales fue diagnosticado de esquizofrenia un conjunto de pacientes[^manual_dsm]:
...
Sea la variable *número de materias aprobadas por estudiantes de segundo año de una carrera universitaria*:
...
Sea la variable *tiempo de reacción a un estímulo visual expresado en segundos* medido en un experimento que tuvo 34 participantes.
...
porque cuando decimos "o inferior" incluimos los valores por debajo del indicado. De lo contrario, si se omite "o menos" se estaría diciendo que la mitad del grupo coincidió *exactamente* en 2.42 segundos.
...
Los cortes que generan 10 grupos con la misma cantidad de casos se llaman **deciles** (cuantiles desde 0.10 hasta 0.90). Por ejemplo, el segundo decil se escribe $D_2$ es el valor de la variable que deja por debajo al 20% de los casos, por lo que coincide con el primer quintil, el percentil 20 y el cuantil 0.20.    
...
```{r fig.cap= "Obtención gráfica de los puntos de corte que delimitan cinco grupos con igual proporción de casos", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
...
ggplot(eph.3.18.asal) +
  ...
  geom_segment(aes(xend = quantile(eph.3.18.asal$PP08D1, .20), yend = 0, x = quantile(eph.3.18.asal$PP08D1, .20), y = .20), arrow = arrow(type = "closed", length = unit(0.05, "inches"))) +
  geom_segment(aes(x = 0, xend = quantile(eph.3.18.asal$PP08D1, .40), y = .40, yend = .40)) +
  geom_segment(aes(xend = quantile(eph.3.18.asal$PP08D1, .40), yend = 0, x = quantile(eph.3.18.asal$PP08D1, .40), y = .40), arrow = arrow(type = "closed", length = unit(0.05, "inches"))) +
  geom_segment(aes(x = 0, xend = quantile(eph.3.18.asal$PP08D1, .60), y = .60, yend = .60)) +
  geom_segment(aes(xend = quantile(eph.3.18.asal$PP08D1, .60), yend = 0, x = quantile(eph.3.18.asal$PP08D1, .60), y = .60), arrow = arrow(type = "closed", length = unit(0.05, "inches"))) +
  geom_segment(aes(x = 0, xend = quantile(eph.3.18.asal$PP08D1, .80), y = .80, yend = .80)) +
  geom_segment(aes(xend = quantile(eph.3.18.asal$PP08D1, .80), yend = 0, x = quantile(eph.3.18.asal$PP08D1, .80), y = .80), arrow = arrow(type = "closed", length = unit(0.05, "inches"))) +
  xlab("ingresos salariales (en pesos)") + ylab("frecuencia relativa acumulada'") + theme_tufte()
```
...
posible hacer uso de las propiedades[^prop_escalas] que tienen estas escalas.
...
```{r fig.cap= "Visualización de valores atípicos", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
ggplot(df1) + geom_bar(aes(x), fill = "green") + xlab("x") + ylab("frecuencia absoluta") + theme_tufte()
```
...
aumento porque el salario promedio del sector es de *X pesos*. Argumento al que se contrapone (expresado de diferentes maneras) que ese promedio incluye al personal que tiene salarios muy altos. Se trata de distribuciones asimétricas, que tienen la mayor parte de los casos con salarios bajos o intermedios y unos pocos casos con salarios muy superiores, como lo muestra el histograma siguiente.  

```{r fig.cap= "Distribución de los ingresos salariales", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
eph.3.18.ingresos <- subset(eph.3.18, eph.3.18$PP08D1 > 0 & is.na(eph.3.18$PP08D1) == FALSE)

ggplot(eph.3.18.ingresos) +
  geom_histogram(aes(PP08D1), fill = "green") +
  xlab("ingresos salariales") +
  ylab("frecuencia absoluta") + theme_tufte()
```
...
```{r fig.cap= "Ejemplo de distribución asimétrica a la izquierda", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
...
```{r fig.cap= "Histogramas de variables con diferente asimetría", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
set.seed(8818)
asim_izq <- rexp(n = 1000, rate = .1)
asim_izq <- max(asim_izq) - asim_izq
df3 <- data.frame(
  sim = rnorm(1000), # simetrica
  asim_der = rexp(n = 1000, rate = .1), # asimétrica a derecha
  asim_izq = asim_izq # asimétrica a izquierda
)
ggplot(df3) + geom_histogram(aes(sim), fill = "green", bins = 30) +
  xlab("x1") + ylab("f") + theme_tufte() + ggtitle("Distribución simétrica") + theme(
...
ggplot(df3) + geom_histogram(aes(asim_der), fill = "green", bins = 30) +
  xlab("x2") + ylab("f") + theme_tufte() + ggtitle("Distribución asimétrica a la derecha") + theme(
    plot.title = element_text(size = 7),
    axis.title = element_text(size = 6),
    axis.text = element_text(size = 5)
  )
...
ggplot(df3) + geom_histogram(aes(asim_izq), fill = "green", bins = 30) +
  xlab("x3") + ylab("f") + theme_tufte() + ggtitle("Distribución asimétrica a la izquierda") + theme(
    plot.title = element_text(size = 7),
    axis.title = element_text(size = 6),
    axis.text = element_text(size = 5)
  )
...
- x1: media = `r round(mean(df3$sim),2)`, mediana = `r round(median(df3$sim),2)`
- x2: media = `r round(mean(df3$asim_der),2)`, mediana = `r round(median(df3$asim_der),2)`
- x3: media = `r round(mean(df3$asim_izq),2)`, mediana = `r round(median(df3$asim_izq),2)`
...
```{r fig.cap= "Ojivas de Galton correspondientes a variables con diferente asimetría", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
...
```{r fig.cap= "Histogramas de variables con diferente curtosis", fig.asp = 0.7, fig.width = 3, fig.align = 'center', warning=FALSE}
df4 <- data.frame(
...
ggplot(df4) + geom_histogram(aes(n1), fill = "green", bins = 30) +
  ...
  xlab("x4") + ylab("f") + ggtitle("Distribución leptocúrtica") + theme_tufte() + theme(
    plot.title = element_text(size = 7),
    axis.title = element_text(size = 6),
    axis.text = element_text(size = 5)
  )
ggplot(df4) + geom_histogram(aes(n2), fill = "green", bins = 30) +
  xlim(100, 300) + ylim(0, 320) +
  xlab("x5") + ylab("f") + ggtitle("Distribución mesocúrtica") + theme_tufte() + theme(
    plot.title = element_text(size = 7),
    axis.title = element_text(size = 6),
    axis.text = element_text(size = 5)
  )
ggplot(df4) + geom_histogram(aes(n3), fill = "green", bins = 30) +
  xlim(100, 300) + ylim(0, 320) +
  xlab("x6") + ylab("f") + ggtitle("Distribución platicúrtica") + theme_tufte() + theme(
    plot.title = element_text(size = 7),
    axis.title = element_text(size = 6),
    axis.text = element_text(size = 5)
  )
...
propuesto por John Tukey en 1970, pero no se hizo conocido hasta la publicación posterior [@tukey77].
Aplicado a las edades de quienes están cursando en la universidad según la EPH, genera:

```{r fig.cap="Box plot de las edades de estudiantes en la universidad", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
...
```{r fig.cap="Box plot de las edades de estudiantes en la universidad, según estado laboral", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
univ$ESTADO <- as.factor(univ$ESTADO)
levels(univ$ESTADO) <- c("ocupades", "desocupades", "inactives")
ggplot(univ) +
...
```{r fig.cap="Box plots correspondientes a variables de diferente asimetría", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
ggplot(df3) + geom_boxplot(aes(1, sim), fill = "green") +
  xlab("x1") + theme_tufte() +
  theme(
    axis.title.y = element_blank(), axis.text.y = element_blank(),
    axis.ticks.y = element_blank()
  ) + xlab("x1") + ggtitle("Distribución simétrica") + coord_flip() + theme(
    plot.title = element_text(size = 7),
    axis.title = element_text(size = 6),
    axis.text = element_text(size = 5)
  )
...
ggplot(df3) + geom_boxplot(aes(1, asim_der), fill = "green") +
  xlab("x2") + theme_tufte() + theme(
    axis.title.y = element_blank(), axis.text.y = element_blank(),
    axis.ticks.y = element_blank()
  ) + xlab("x2") + ggtitle("Distribución asimétrica a la derecha") + coord_flip() + theme(
    plot.title = element_text(size = 7),
    axis.title = element_text(size = 6),
    axis.text = element_text(size = 5)
  )
...
ggplot(df3) + geom_boxplot(aes(1, asim_izq), fill = "green") +
  xlab("x3") + theme_tufte() + theme(
    axis.title.y = element_blank(), axis.text.y = element_blank(),
    axis.ticks.y = element_blank()
  ) + xlab("x3") + ggtitle("Distribución asimétrica a la izquierda") + coord_flip() + theme(
    plot.title = element_text(size = 7),
    axis.title = element_text(size = 6),
    axis.text = element_text(size = 5)
  )
...
```{r fig.cap="Esquema de la ubicación de la media en el punto de equilibrio del histograma", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
ggplot(univ) + geom_histogram(aes(CH06), bins = 30, fill = "green") +
  geom_point(
    size = 2, stroke = 1.4,
    x = mean(univ$CH06), y = -20, col = "red", shape = 17
  ) +
  xlab("edad") + ylab("f") + theme_tufte() + ylim(c(-30, 750)) + theme(
    plot.title = element_text(size = 7),
    axis.title = element_text(size = 6),
    axis.text = element_text(size = 5)
  )
...
inconvenientes. Uno es que sus unidades están elevadas al cuadrado; por lo que, si medimos *número de errores*, la varianza quedará expresada en *número de errores al cuadrado* una entidad que no tiene significado, como tampoco lo tienen *hijos o hijas al cuadrado* para la fecundidad, *pesos al cuadrado* para ingresos o *segundos al cuadrado* para los tiempos de reacción.
...
unidades de *s* son las mismas que las de la variable original y se resuelve el primer problema.  
...
operaciones que involucra. Se trata de seis casos clínicos diagnosticados de depresión a partir de cinco o más de los síntomas que indica el manual DSM IV[^sint_dsm] y que para cada uno de ellos observamos (como variable) el número de síntomas que condujeron al diagnóstico:

| Caso | $x_{i}$ (número de síntomas) | $x_{i} - \overline{x}$ (desvíos) | ${{(x}_{i} - \overline{x})}^{2}$ (cuadrados de los desvíos) |
|:--------:|:----------------------------:|:--------------------------------:|:-------------------------------------------------------------------------:|
| 1 | 5 | -2 | 4 |
| 2 | 6 | -1 | 1 |
| 3 | 6 | -1 | 1 |
...
@tukey77 analiza la distancia de los casos a los cuartiles y sugiere
...
```{r fig.cap="Ubicación visual de observaciones lejanas por medio del box plot", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
...
Esta medida se llama *entropía de Shannon*, @shannon1948. El cálculo consiste en multiplicar cada frecuencia relativa por su
...
refiere al valor de la variable en un *caso*, por
...
| Caso | $x_{i}$ (número de síntomas) | $x_{i} - \overline{x}$ (desvíos) | $z$ (desvíos estándar) |
|:--------:|:----------------------------:|:--------------------------------:|:----------------------:|
| 1 | 5 | -2 | -1.29 |
| 2 | 6 | -1 | -0.65 |
| 3 | 6 | -1 | -0.65 |
...
en el intervalo de **una** desviación estándar de la media se encuentra aproximadamente el 68% de los casos, a **dos** desviaciones el 95 y a **tres**, el 99%. Este resultado se conoce como **regla empírica**.  
...
```{r fig.cap="Distribución no unimodal: la desigualdad de Chebycheff establece un mínimo para la proporción de casos que hay a dos desviaciones estándar de la media", fig.asp = 0.7, fig.width = 3, fig.align = 'center'}
ggplot(dos.hist) +
  geom_histogram(aes(unif), fill = "green", bins = 30) +
  geom_point(
    size = 3, stroke = 1.4,
    x = mean(unif) - 1.5 * sd(unif), y = -2, col = "red", shape = 2
  ) +
  geom_point(
    size = 3, stroke = 1.4,
    x = mean(unif) + 1.5 * sd(unif), y = -2, col = "red", shape = 2
  ...
  ylab("f") +
  geom_segment(x = mean(unif) - 1.5 * sd(unif), xend = mean(unif) - 1.5 * sd(unif), y = 0, yend = 40) +
  geom_segment(x = mean(unif) + 1.5 * sd(unif), xend = mean(unif) + 1.5 * sd(unif), y = 0, yend = 40) +
  geom_text(aes(label = "al menos 75% de \n las observaciones \n se encuentra en esta zona", x = mean(unif), y = 20), size = 3) +
  xlab("") +
  theme_tufte()
```
...
```{r fig.cap="Distribución unimodal y simétrica: la regla empírica ofrece una aproximación la proporción de casos que hay a dos desviaciones estándar de la media", fig.asp = 0.7, fig.width = 4, fig.align = 'center'}
ggplot(dos.hist) + geom_histogram(aes(norm), fill = "green", bins = 30) + ylab("f") +
  geom_point(
    size = 3, stroke = 1.4,
    x = mean(norm) - 2 * sd(norm), y = -2, col = "red", shape = 2
  ) +
  geom_point(
    size = 3, stroke = 1.4,
    x = mean(norm) + 2 * sd(norm), y = -2, col = "red", shape = 2
  ) +
  geom_segment(x = mean(norm) - 2 * sd(norm), xend = mean(norm) - 2 * sd(norm), y = 0, yend = 40) +
  geom_segment(x = mean(norm) + 2 * sd(norm), xend = mean(norm) + 2 * sd(norm), y = 0, yend = 40) +
  geom_text(aes(label = "aproximadamente el 95% \n de las observaciones se \n encuentra en esta zona", x = mean(norm), y = 30), size = 3) + xlab("") + theme_tufte()
```
...
```{r echo=TRUE}
eph.3.18 <- read.table(
  "bases/archivostxt/usu_individual_T318.txt",
  header = TRUE, sep = ";"
)
```

Ahora retendremos solo a las personas que trabajan por un salario (CAT_OCUP = 3) que tienen ingreso mayor a cero (PP08D1>0 y que no son NA), la nueva base se llama "asalariades":
```{r echo=TRUE}
asalariades <- subset(eph.3.18, eph.3.18$CAT_OCUP == 3 &
...
length(asalariades$PP08D1) # la cantidad de casos (n)
mean(asalariades$PP08D1) # la media
median(asalariades$PP08D1) # la mediana
```
...
```{r, echo=TRUE}
quantile(asalariades$PP08D1, .2) # cuantil 0.20 ó percentil 20
quantile(asalariades$PP08D1, .05) # percentil 5
quantile(asalariades$PP08D1, .95) # percentil 95
quantile(asalariades$PP08D1, .5) # percentil 50, coincide con la mediana
```

Si se necesitan varios juntos, por ejemplo los quintiles, se usa el comando `c`, para "concatenar" los valores:

```{r, echo=TRUE}
quantile(asalariades$PP08D1, c(.2, .4, .6, .8))
```

Esta descripción de los datos se lee:
En la muestra de `r length(asalariades$PP08D1)` personas que trabajan en relación de dependencia, el salario promedio es de `r format(mean(asalariades$PP08D1), scientific = FALSE)`. El 50% tiene salario de `r format(median(asalariades$PP08D1), scientific = FALSE)`, el 20% de más bajos salarios está de `r format(quantile(asalariades$PP08D1,.2), scientific = FALSE)` para abajo y el 5% de los menores salarios son de `r format(quantile(asalariades$PP08D1,.05), scientific = FALSE)` o menos. El 95% tiene salario de `r format(quantile(asalariades$PP08D1,.95), scientific = FALSE)` o menos; que es lo mismo que decir que el 5% de los salarios superan los `r format(quantile(asalariades$PP08D1,.95), scientific = FALSE)`.

Repitamos la categorización con los tres criterios vistos antes, por medio del comando `cut` que se aplica sobre una variable cuantitativa y se controla la cantidad de intervalos y la forma de construirlos en el argumento `breaks`. Para categorizar la edad de la base EPH en cuatro intervalos de igual amplitud, se indica ese número en el argumento. El resultado de esa categorización se guarda en una nueva variable a la que llamamos edad_4_igual y luego se solicita su distribución de frecuencia:

```{r, echo=TRUE, warning=FALSE, message=FALSE}
edad_4_igual <- cut(eph.3.18$CH06, breaks = 4)

library("summarytools")

freq(edad_4_igual, report.nas = FALSE)
```
...
```{r, echo=TRUE}
edad_4_prop <- cut(eph.3.18$CH06, breaks = c(
  0,
...
freq(edad_4_prop, report.nas = FALSE)
```
...
```{r, echo=TRUE}
edad_4_teo <- cut(eph.3.18$CH06, breaks = c(
  0,
  14,
  44,
  64,
...
freq(edad_4_teo, report.nas = FALSE)
```
...
R <- max(asalariades$PP08D1) -
  min(asalariades$PP08D1) # se define R
names(R) <- "R" # se le da un nombre
round(R, 2) # se lo redondea a dos decimales

AIQ <- quantile(asalariades$PP08D1, .75) -
  quantile(asalariades$PP08D1, .25)
names(AIQ) <- "AIQ"
round(AIQ, 2)

SRIC <- (quantile(asalariades$PP08D1, .75) -
           quantile(asalariades$PP08D1, .25)) / 2
names(SRIC) <- "SRIC"
round(SRIC, 2)

var <- var(asalariades$PP08D1) # el cálculo de la varianza está en
# el paquete {base}
names(var) <- "s2"
round(var, 2)

s <- sd(asalariades$PP08D1) # también la desviación estándar
names(s) <- "s"
round(s, 2)

# al CV hay que definirlo
CV <- sd(asalariades$PP08D1) / mean(asalariades$PP08D1) * 100 
...
```{r warning= FALSE, echo=TRUE}
cv_motora <- round(
  100 * sd(bayley$baymotor) / mean(bayley$baymotor), 1)

cv_mental <- round(
  100 * sd(bayley$bayment) / mean(bayley$bayment), 1)
```
...
[^sint_dsm]: Presencia de cinco (o más) de los siguientes síntomas durante un período de 2 semanas, que representan un cambio respecto a la actividad previa; uno de los síntomas debe ser (textualmente):
