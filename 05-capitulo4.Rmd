...
kableExtra::kable_styling(
  knitr::kable(aux_table,
    format = "pandoc", booktabs = TRUE, align = "lccc",
    caption = "Disposición de las variables para el análisis de su relación."
  ),
...
Cuando se distribuyen los datos en las celdas, se obtiene una **tabla bivariada** (porque contiene dos variables), que también se llama **tabla de contingencia** o **tabla de distribución conjunta** o **también tabla de doble entrada**. Las celdas del interior de la tabla llevarán, cuando los datos sean recolectados, la cantidad de personas que se encuentren en cada coincidencia de categorías. Si nuestra hipótesis afirmara que quienes provienen de hogares nucleares son más propensos a ser líderes, esperaríamos una concentración relativa de casos en la celda correspondiente a "hogar nuclear" - "liderazgo", hipótesis que luego deberemos confrontar con la información recogida.
...
kableExtra::kable_styling(
  knitr::kable(aux_table,
    ...
    caption = "Notación para las frecuencias en una tabla bivariada."
  ),
  latex_options = "striped"
)
```
...
kableExtra::kable_styling(
  knitr::kable(aux_table,
    format = "pandoc", booktabs = TRUE, align = "lcccc",
    caption = "Estudiantes de escuelas primarias por relación con sus pares según tipo de hogar."
  ),
  latex_options = "striped"
)
```
...
kableExtra::kable_styling(
  knitr::kable(aux_table,
    format = "pandoc", booktabs = TRUE, align = "lcccc",
    label = "hogaresrel",
    caption = "Estudiantes de escuelas primarias por relación con sus pares según tipo de hogar, frecuencias relativas."
  ),
  latex_options = "striped"
)
```
...
Si se observa que una variable cambia *a continuación* de la otra (en
sentido temporal) esto no quiere decir que cambia *a causa*
de la otra.
...
superior, se puede plantear la **dirección** de la relación.
...
```{r fig.cap="Relación hipotética entre la ayuda en el hogar y el rendimiento escolar", fig.asp = 0.1, fig.width = 3, fig.align='center'}
...
kableExtra::kable_styling(
  knitr::kable(aux_table, format = "pandoc", booktabs = TRUE, align = "lcccc", caption = "Ubicación de las variables para analizar la relación entre la ayuda familiar con las tareas y el rendimiento que alcanzan en la escuela"),
  latex_options = "striped"
)
```
...
interpreta que las personas que son hipertensas tienen dos veces y media mayores posibilidades de sufrir CI que quienes no lo son. El riesgo relativo se usa principalmente (pero no solo) en la relación entre dos variables dicotómicas; en este ejemplo las variables dicotómicas son: tener hipertensión crónica y haber sufrido cardiopatía isquémica, ambas con categorías si - no, a las que denominamos: hip, nohip y card, nocard, respectivamente. La primera es el antecedente y la segunda el consecuente, porque interesa plantear el posible efecto de la hipertensión sobre la CI. A partir de los datos obtenidos en el estudio de @RamonGonzalezJuanatey2001, la relación se plantea así:  
...
kableExtra::kable_styling(
  knitr::kable(tabla21, format = "pandoc", booktabs = TRUE, align = "ccc", caption = "Distribución conjunta de hipertensión crónica y cardiopatía isquémica"),
  latex_options = "striped"
)
```
...
kableExtra::kable_styling(
  knitr::kable(tabla22, format = "pandoc", booktabs = TRUE, align = "ccc", caption = "Distribución conjunta de hipertensión crónica y cardiopatía isquémica (frecuencia relativas por filas)"),
  latex_options = "striped"
)
```
...
```{r}
one <- read.csv("bases/archivostxt/ONE_2000_cordoba.txt", sep = ";")
one$P16<-as.factor(one$P16)
one<-subset(one, one$P16=="1" | one$P16=="2")
one$P16<-droplevels(one$P16) 
levels(one$P16)<- c("si", "no")
levels(one$CATRENLEN) <- c(NA, "bajo", "alto")
one$CATRENLEN <- factor(one$CATRENLEN)
lectura_rendim_absolutas <- addmargins(table(one$P16, one$CATRENLEN))
dimnames(lectura_rendim_absolutas)[[1]][3] <- "Total"
dimnames(lectura_rendim_absolutas)[[2]][3] <- "Total"

lectura_rendim_relativas <- round(addmargins(prop.table(table(one$P16, one$CATRENLEN), 1), 2), 2)
dimnames(lectura_rendim_relativas)[[2]][3] <- "Total"
...
kableExtra::kable_styling(
  knitr::kable(aux_table, format = "markdown", booktabs = TRUE, align = "lccc", caption = "Distribución conjunta de la lectura extraescolar y el resultado en la prueba de lengua"),
  latex_options = "striped"
) 
```
...
Además de la simetría o asimetría y de la dirección (si se la puede indicar), hay otra característica que hace falta medir de las relaciones entre variables, que es su **intensidad**. Esta medida de la relación es lo que está implícito cuando se dice: "X tiene mucha influencia en Y", la idea de mucha o poca influencia es la de intensidad de la relación. Cuando hay muchos factores explicativos
...
kableExtra::kable_styling(
  knitr::kable(aux_table,
    format = "pandoc", booktabs = TRUE, align = "lcccc",
    caption = "Estudiantes de escuelas primarias por rendimiento según tipo de docente, frecuencias absolutas"
  ),
  latex_options = "striped"
)
```
...
El 36.7% de las personas encuestadas considera que su país está estancado **y** no considera la posibilidad de emigrar.
...
Este valor indica que no hay relación entre las variables, es decir que, según estos datos, la opinión no haría diferencia alguna sobre la intención de emigrar. En una situación así se considera a las dos variables como **estadísticamente independientes**, nuevamente, es un caso que no se observa con datos reales.  
...
```{r}
tabla18 <- round(prop.table(addmargins(round(ji$expected, 0), 2), 2), 3)
...
```{r echo=TRUE, warning=FALSE}
levels(eph.3.18.validos.estado$ESTADO) <- c(
...
```{r echo=TRUE, warning=FALSE}
eph.3.18.validos.estado$sexo <- as.factor(
  eph.3.18.validos.estado$CH04
)
```
...
```{r echo=TRUE, warning=FALSE}
levels(eph.3.18.validos.estado$sexo) <- c(
  "varones", "mujeres"
)
```
...
```{r}
sexo_por_estado <- table(
  eph.3.18.validos.estado$sexo,
  eph.3.18.validos.estado$ESTADO
)
```


Los totales de filas y de columna (frecuencias marginales) deben pedirse explícitamente, con `addmargins`, lo aplicamos sobre la tabla que ya está guardada:

```{r warning=FALSE, echo=TRUE}
addmargins(sexo_por_estado)
```

Así como las relativas, usando `prop.table`:

```{r warning=FALSE, echo=TRUE}
prop.table(sexo_por_estado)
```

Para reducir los decimales, se solicita redondeo a solo tres:

```{r warning=FALSE, echo=TRUE}
round(prop.table(sexo_por_estado), 3)
```

Y para expresarlos como porcentajes, lo multiplicamos por 100:

```{r warning=FALSE, echo=TRUE}
round(prop.table(sexo_por_estado), 3) * 100
```

Estas frecuencias relativas, lo son respecto del total, si se las quiere por filas, se indica en el argumento *1*

```{r warning=FALSE, echo=TRUE}
round(prop.table(sexo_por_estado, 1), 3) * 100
```

Para obtenerlas por columnas, se indica *2* en el argumento de `prop.table`:

```{r warning=FALSE, echo=TRUE}
round(prop.table(sexo_por_estado, 2), 3) * 100
```
...
Por defecto calcula relativas por filas. Si las queremos por columnas, se indica como `prop = "c"`:

```{r warning=FALSE, echo=TRUE, eval=FALSE}
ctable(eph.3.18.validos.estado$sexo,
  eph.3.18.validos.estado$ESTADO,
  prop = "c"
)
```
...
Si solo se quieren las frecuencias absolutas, `prop = "n"`:
